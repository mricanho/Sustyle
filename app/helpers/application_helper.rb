module ApplicationHelper
  def four_articles(article, index)
    if index < 2
      content_tag(:div, nil, class: 'photo', style: "background: url('#{display_photo(article)}') center center; background-size: cover") do
      end +
  ...
  def display_details(article)
    content_tag(:h1, @category.name, class: 'orange hero-head title is-size-4') + content_tag(:h2, article.title, class: 'title is-size-4') +
